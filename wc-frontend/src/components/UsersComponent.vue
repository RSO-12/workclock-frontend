      ...
      <v-container class="pa-0 ma-0 mb-5" style="width: 100%">
        <v-row class="justify-space-between">
          <v-col class="text-left d-flex flex-row align-center">
            <h3>Users</h3>
            <v-icon class="ml-2" color="green" @click="openAddUserDialog()">mdi-account-plus</v-icon>
          </v-col>
          <v-col class="text-right justify-end align-center">
            <v-text-field class="flex-full-width" density="comfortable" placeholder="Search"
  ...
  <v-dialog class="add-user-modal-dialog" v-model="editUserDialog">
    <v-card class="add-user-modal-card pa-5">
      ...
      <v-text-field v-model="name" placeholder="John Doe" variant="solo" rounded="lg" bg-color="rgba(255, 255, 255, 0.02)"
        :rules="nameRules()" clearable :counter="20" maxlength="20" :error-messages="errors.name"></v-text-field>
      <div class="pl-1 text-subtitle-2 text-medium-emphasis text-left">
        EMAIL
      </div>
      ...
      <v-text-field v-model="password" placeholder="not_123456" type="password" variant="solo" rounded="lg"
        bg-color="rgba(255, 255, 255, 0.02)" :rules="passwordRules()" :error-messages="errors.password"></v-text-field>
      <div class="pl-1 text-subtitle-2 text-medium-emphasis text-left">
        CONFIRM PASSWORD
      </div>
      ...
      <v-card-actions class="d-flex justify-end">
        <v-btn color="primary" variant="flat" @click="openEditUserDialog(), clearData()">Cancel</v-btn>
        <v-btn color="primary" variant="flat" @click="openEditUserDialog(), editUser()">Edit user</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
